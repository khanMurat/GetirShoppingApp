...
final class BasketProductCollectionViewCell : UICollectionViewCell {
    ...
    private func addProductBasket(){
        self.stepperView.didIncrease = { [weak self] in
            guard let self = self else {return}
            self.cellPresenter.addProductToBasket()
        }
    }
    
    private func removeProductFromBasket(){
        self.stepperView.didDecrease = { [weak self] in
            guard let self = self else {return}
            self.cellPresenter.removeProductFromBasket()
        }
    }
}
...
extension BasketProductCollectionViewCell : BasketProductProtocol {
    ...
    func setProductBasketCount(_ count: Int) {
        stepperView.productCount = count
    }
}
